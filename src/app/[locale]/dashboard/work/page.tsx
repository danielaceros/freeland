...
export default function Work() {
      ...
      for (const userDoc of usersSnapshot.docs) {
        ...
        for (const offerDoc of offersSnapshot.docs) {
          offersData.push({
            ...
            durationValue: parseInt(offerDoc.data().durationValue),
  ...
  const filteredOffers = offers.filter((offer) => {
    const matchesPrice = offer.priceHour! >= minPrice && offer.priceHour! <= maxPrice;
    const matchesDuration = offer.durationValue! >= minDuration && offer.durationValue! <= maxDuration;
    const matchesSkills = selectedSkills.every((skill: string) => offer.skillsMin!.includes(skill));
    const matchesSearchTerm =
      offer.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      offer.description.toLowerCase().includes(searchTerm.toLowerCase());
    return matchesPrice && matchesDuration && matchesSkills && matchesSearchTerm;
  });
            ...
            <>
              {user ? (
                <div className="flex flex-col">
                  ...
                  {/* Duration Filter */}
                  <div className="mb-4">
                    <h4 className="text-lg font-semibold">{t('durationRange')}</h4>
                    <Slider
                      range
                      min={1}
                      max={48}
                    ...
                    />
                    <div className="flex justify-between">
                      <span>{minDuration} {t('months')}</span>
                      <span>{maxDuration} {t('months')}</span>
                    </div>
                  </div>
                  
                  {/* Skills Filter */}
                  <div className="mb-4">
                    <h4 className="text-lg font-semibold">{t('requiredSkills')}</h4>
                    <Select
                      isMulti
                ...
                {modalOpen && selectedOffer && (
                  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="w-full max-w-4xl rounded-lg bg-white p-8 shadow-lg overflow-y-auto max-h-[90vh]">
                      <div className="flex justify-between">
                        <h2 className="text-2xl font-semibold">{selectedOffer.name}</h2>
                      ...
                      <div className="mt-8 grid grid-cols-2 gap-8">
                        ...
                        <div>
                        <h5 className="text-lg font-semibold">{t('publishDate')}</h5>
                        <p className="mb-5 font-semibold">{new Date(selectedOffer.createdAt).toLocaleDateString()}</p> 
                          {offerPow ? (
                            <div className="flex items-center">
                              <button
                                onClick={handleViewRecruiterPoW}
                                className="w-full mt-4 rounded bg-blue-600 px-4 py-2 text-white"
                              >
                                {t('viewProofOfWork')}
                              </button>
                            </div>
                          ...
                          {uploadPow ? (
                            ...
                            <div className="mt-4">
                                ...
                                <div className="h-48 border-2 border-dashed border-gray-300 bg-gray-50">
                                  <div {...getRootProps()} className="flex justify-center items-center h-full">
                                    <input {...getInputProps()} />
                                    {previewUrl ? (
                                      <div className="text-center">
                                        <img
                                          src={previewUrl}
                                          alt="Preview"
                                          className="mx-auto max-w-full max-h-32"
                                        />
                                      </div>
                                    ) : (
                                      <p className="text-gray-500">{t('dragOrClickToSelectFile')}</p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <button
                                  ...
                                  className="w-full mt-4 rounded bg-green-600 px-4 py-2 text-white"
                                >
                                  {t('uploadProofOfWork')}
                                </button>
                            </div>
                      ...
                      <div>
                          <h3 className="mb-4 text-lg font-semibold">{t('requiredSkills')}</h3>
